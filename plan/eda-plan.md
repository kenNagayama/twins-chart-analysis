...
2. (テスト用) 一般的な統計処理を試す
  - スライドウィンドウでチャートがどう変わるかをテストする
    - ウィンドウ幅は可変にする
    - RMS処理を試す
    - FFT処理を試す
    - STFT処理を試す
3. (本命) データの前処理とノイズフィルタリング
   - 計測エラーによるスパイクノイズを除去する
     - ノイズ除去手法の候補
       - 移動中央値フィルタ
       - サビツキー・ゴーレイ法による平滑化
   - 2つの手法について、 **ON/OFF** をスイッチできるようにする
4. 移動平均からの乖離（Z-Score）を計算して、急激な直径低下を検知します
5. 結果を **インタラクティブなグラフとして可視化** し、元の波形、フィルタ後、異常検知箇所を上下に並べて比較できるようにします。
